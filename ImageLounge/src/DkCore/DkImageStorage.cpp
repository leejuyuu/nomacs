...
static cv::Mat exposureLut(double exposure)
{
    ...
    for (int i = 0; i < lut.cols; i++) {
        ...
        if (valE < 0)
            ...
            ptrLut[i] = (unsigned short)maxVal;
        else
            ptrLut[i] = (unsigned short)qRound(valE);
    }
    return lut;
}
...
static cv::Mat gammaLut(double gamma)
{
    ...
    for (int i = 0; i < lut.cols; i++) {
        double val = gamma <= 0.0 ? 0.0 : std::pow((double)i / maxVal, 1.0 / gamma) * maxVal;
        ptrLut[i] = (unsigned short)qRound(val);
    }

    return lut;
}

static cv::Mat offsetLut(double offset)
{
    int maxVal = std::numeric_limits<unsigned short>::max();
    cv::Mat lut(1, maxVal + 1, CV_16UC1);
    auto *ptrLut = lut.ptr<unsigned short>();
    ...
    int k = offset * maxVal;
    for (int i = 0; i < lut.cols; i++) {
        ptrLut[i] = qBound(0, i + k, maxVal);
    }

    return lut;
}

static cv::Mat combineLuts(const cv::Mat &a, const cv::Mat &b)
{
    int maxVal = std::numeric_limits<unsigned short>::max();
    cv::Mat out(1, maxVal + 1, CV_16UC1);
    auto *ptrOut = out.ptr<unsigned short>();
    auto *ptrA = a.ptr<unsigned short>();
    auto *ptrB = b.ptr<unsigned short>();

    for (int i = 0; i < out.cols; i++) {
        ptrOut[i] = ptrB[ptrA[i]];
    }
    return out;
}
...
protected:
    ...
    static bool kernel(const std::any &arg, const DkWorkRange &range)
    {
        ...
        Q_ASSERT(lut.rows == 1 && lut.cols == (U16_Max + 1) && lut.type() == CV_16UC1);
...
std::optional<QImage> DkImage::exposure(QImage &&img, double exposure, double offset, double gamma)
{
    ...
    cv::Mat lut = offsetLut(offset);

    lut = combineLuts(lut, gammaLut(gamma));
...
{
    return DkConstNativeImage::fromImage(img, DkNativeImage::map_bgr).constMat().clone();
}
...
QImage DkImage::tinyPlanet(const QImage &img, double scaleLog, double angle, const QSize &size, bool invert)
{
    ...
    auto view = DkNativeImage::fromImage(std::move(tmp));
