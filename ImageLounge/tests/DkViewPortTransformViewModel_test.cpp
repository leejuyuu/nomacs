...
// NOTE: this is created because EXPECT_DOUBLE_EQ is strict,
// while qFuzzyCompare has some issues around 0 since it is purely relative.
template<typename T>
testing::AssertionResult assertClose(const T &actual, const T &expected)
{
...
TEST_P(SetWidgetSizeTest, SetWidgetSize)
{
    ...
    EXPECT_TRUE(assertClose(vm->getImageViewRect().size(), params.imageSize * expectedZL))
        << "unequal getImageViewRect size";
    EXPECT_TRUE(assertClose(vm->getImageViewRect().topLeft(), params.expectedImageViewRectTopLeft))
...
TEST_P(SyncTest, SyncTransform)
{
    ...
    EXPECT_TRUE(assertClose(srcFinalRel, tgtFinalRel)) << "unequal relative center after sync op";
}
...
INSTANTIATE_TEST_SUITE_P(AllPolicies,
                         ...
                         testing::ValuesIn(std::vector<PanRestrictionTestParam>{
                             // Large Image (200x150 in 100x80 viewport)
                             // Initial TopLeft is (-50, -35)
                             {
                                 "large image, free",
                             ...
                             // Small Image (50x60 in 100x80 viewport)
                             // Initial TopLeft is (25, 10)
                             {
                                 "small image, free",
                                 DkViewPortTransformViewModel::PanBoundary::None,
